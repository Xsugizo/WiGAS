...
def retry(session,action):
    ...
    if coun=='1' or action!='y':
        if coun=='1':
            send_mail('Yaqing_Wu@wistron.com,Ian_H_Chang@wistron.com,Jonathan_Tung@wistron.com', 'WiGAS retry setting', 'Error! Please check device Qty.')
        ...
        if action=='y':
            ...
            #     f.write("#!/bin/bash"+'\n')
            #     f.write("cd /home/logo113/Desktop/IMAGE/workspace/Pipeline_Testing/11_r10/android-cts-11_r10-linux_x86-arm/android-cts/tools"+'\n')
            #     f.write("./cts-tradefed run retry --retry "+str(session)+" "+cmd+'\n')
            # f.close()
        else:
            retry(session,action)
    else:
        with open('/home/logo113/Desktop/IMAGE/workspace/Pipeline_Testing/retry1.sh','w')as f:       
