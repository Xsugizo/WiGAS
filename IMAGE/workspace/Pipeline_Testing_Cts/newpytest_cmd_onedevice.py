...
from check_productname_image import check_product
...
def get_latestfolder():
    ...
    # latestfolder=""
    checkfolder = False
    directory = dirpath+'/android-gts/results'
    flagg=0
    ...
    for i in latest:
        if i == dirpath+'/android-gts/results/latest':
            latest.remove(dirpath+'/android-gts/results/latest') 
...
def firstII(cts):
    ...
    p_pid=p.pid
    time.sleep(5)
    p_pid = subprocess.check_output('pgrep -l -u logo113 | grep cts-tradefed | tail -1', shell=True) # get the process id
...
def retry_timeII(r1):
    print("retry_timeII")
    android_home = dirpath
    gts_command = android_home + "/android-gts/tools/cts-tradefed"
    r1=gts_command +" "+r1
    print("r="+r1)
    # check =input('Please comfirm all devices have entered fastboot mode [y/n] ...')
    pp=subprocess.Popen(f"gnome-terminal -- bash -c '{listToString(r1)}'", shell=True)
    pp_pid=pp.pid
    time.sleep(5)
    pp_pid = subprocess.check_output('pgrep -l -u logo113 | grep cts-tradefed | tail -1', shell=True) # get the process id
...
count = 0
r1 = ''
product_name_status=check_product()
if product_name_status=="same_product_name":
    while (retry==1):
   ...
   print("please re-range your devices")
