...
cts_tradefed_script=dirpath+'/android-cts/tools/'
run_turn  = 87
run_reboot = 1
run_factory = 1
...
#change cmd path 
os.chdir(path)
runcode ='/home/logo113/Desktop/IMAGE/workspace/Pipeline_Testing/runcts.sh'
#get devices image and run shell script
def run_code(image,runcode):
    #get image name
    out_image = os.popen(image).read()
    #print(out_image)
    out_image = out_image.split('/')
    os.chdir(cts_tradefed_script)
    #run shell
    shellscript = subprocess.Popen(runcode,stdin=None,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    #subprocess.run(['./cts-tradefed','run', 'cts', '-m', 'CtsCameraTestCases', '--shard-count', '2', '-s', '212555225E0089' ,'-s', '212555225E0147'])
    #os.system('exit | ./cts-tradefed run cts -m CtsCameraTestCases --shard-count 2 -s 212555225E0089 -s 212555225E0147')
    print("wait for run...")
